...
class Labyrinth:
    ...
    def __move_states_by_coords(self, coords: tuple) -> bool:
        newstates = []
    ...
    def heuristic(self) -> int:
        max_length = 0
        for state in self.states:
            max_length = max(max_length, self.opt_lengths[state])
        return max_length
...
def bfs(labopt: Labyrinth, goals: list) -> list:
    ...
    queue.put((0 + labopt.heuristic(), labopt, []))
    ...
    while not queue.empty():
        ...
        if l_lab not in visited_locs:
            ...
            queue.put((len(l_path) + l_lab.heuristic(), l_lab, l_path))
        if r_lab not in visited_locs:
            visited_locs.add(r_lab)
            queue.put((len(r_path) + r_lab.heuristic(), r_lab, r_path))
        if d_lab not in visited_locs:
            visited_locs.add(d_lab)
            queue.put((len(d_path) + d_lab.heuristic(), d_lab, d_path))
        if u_lab not in visited_locs:
            visited_locs.add(u_lab)
            queue.put((len(u_path) + u_lab.heuristic(), u_lab, u_path))
