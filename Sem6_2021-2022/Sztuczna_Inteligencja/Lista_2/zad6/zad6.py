...
def bfs(labopt: Labyrinth, goals: list) -> list:
    ...
    queue.put((0 + labopt.heuristic(2), labopt, []))
    ...
    while not queue.empty():
        ...
        if l_lab not in visited_locs:
            ...
            queue.put((len(l_path) + l_lab.heuristic(2), l_lab, l_path))
        if r_lab not in visited_locs:
            visited_locs.add(r_lab)
            queue.put((len(r_path) + r_lab.heuristic(2), r_lab, r_path))
        if d_lab not in visited_locs:
            visited_locs.add(d_lab)
            queue.put((len(d_path) + d_lab.heuristic(2), d_lab, d_path))
        if u_lab not in visited_locs:
            visited_locs.add(u_lab)
            queue.put((len(u_path) + u_lab.heuristic(2), u_lab, u_path))
