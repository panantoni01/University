...
class Jungle:
    ...
    def copy(self):
        ...
        new.pieces = copy.deepcopy(self.pieces)
        new.curplayer = self.curplayer
        new.peace_counter = self.peace_counter
    ...
    def simulate(self, player):
        # this function shall be called from a newly created Jungle obj
        curr_player = 1-player
        K = 1
        while True:
            rand_mv = self.random_move(player)
            self.do_move(rand_mv)
            if self.victory(player):
    ...
    def aka_mcts(self, player):
        N = 20000
        children = []
        for mv in self.moves(player):
            children.append([0, mv])
        if not children:
            return None
        ...
        while N > 0:
            new_state = self.copy()
            new_state.do_move(children[i][1])
            (K, winner) = new_state.simulate(1-player)
            children[i][0] += winner
            N -= K
            i = (i+1) % len(children)
...
for _ in range(10):
    ...
    while True:
        ...
        if J.victory(player):
            ...
            print("Result:", "WIN," if player == 1 else "LOST,", "time:", end-start)
            J.draw()
            break
        player = 1 - player
