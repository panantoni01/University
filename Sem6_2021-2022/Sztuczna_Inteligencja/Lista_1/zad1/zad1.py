...
def get_move(pos1: Position, pos2: Position) -> str:
    if pos1.b_king != pos2.b_king:
        return "K" + pos2.b_king
    elif pos1.w_king != pos2.w_king:
        return "K" + pos2.w_king
    return "R" + pos2.w_rook


def print_moves(positions: list) -> None:
    # black moves first
    if len(positions) % 2 == 1:
        print(f"1. ...   {get_move(positions[0], positions[1])}")
        start = 2
    # white moves first
    else:
        start = 1
    for i in range(start, len(positions) - 1, 2):
        print(f"{i//2 + 1}. {get_move(positions[i-1], positions[i])}   {get_move(positions[i], positions[i+1])}")
    print(f"{len(positions)//2 + 1}. {get_move(positions[-2], positions[-1])}#")


with open("zad1_input.txt", "r") as infile:
    with open("zad1_output.txt", "w") as outfile:
        for line in infile:
            list = line.split()
            pos = Position(list[0], list[1], list[2], list[3])
            if pos.is_checkmate():
                result = str(print(0)) + "\n"
            elif not pos.possible_moves():
                result = "INF\n"
            else:
                positions = bfs(pos, set())
                result = str(len(positions) - 1)
                if DEBUG:
                    print(f"Starting position: {line}")
                    print_moves(positions)
                    print(f"Checkmate after {result} moves")
